"""Tests for bambox.auth — login flows, profile, device discovery."""
...
class TestLoginVerificationCodeFlow:
    def test_verify_code_flow(self, monkeypatch):
        monkeypatch.setattr("bambox.auth.getpass.getpass", lambda prompt: "123456")
    ...
    def test_verify_code_flow_no_token_raises(self, monkeypatch):
        monkeypatch.setattr("bambox.auth.getpass.getpass", lambda prompt: "000000")
...
class TestLoginTfaFlow:
    def test_tfa_flow(self, monkeypatch):
        monkeypatch.setattr("bambox.auth.getpass.getpass", lambda prompt: "654321")
    ...
    def test_tfa_http_error(self, monkeypatch):
        monkeypatch.setattr("bambox.auth.getpass.getpass", lambda prompt: "123")
